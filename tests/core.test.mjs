import { SanaStore, memoryStorage, STORAGE_KEY, calculateRating, previewRating, assessText, emptyFields, FIELD_KEYS, levelFor, analyzeDraft, validateAIOutput, prepareInput, DEMO_CARD, PARTICIPANT_CARDS, SEED_DRAFTS, K1_ANSWERS, k1AnswerPatch, PROPOSAL_EXAMPLES, seedDemo } from '../js/index.mjs';
...
test('Контекст и конкретные критерии дают 10 + 15 баллов', () => equal(calculateRating({ context: DEMO_CARD.context, success_metric: DEMO_CARD.success_metric }, ['context', 'success_metric']).score, 25));
...
test('Каталог первого участника сортируется и фильтруется по новой шкале', () => { const s = newStore(); seedDemo(s); equal(s.catalog().map(t => t.rating.score), [93,85,70,65,35]); equal(s.catalog({ level: 'draft' }).length, 1); equal(s.catalog({ topic: 'логистика' }).length, 1); equal(s.catalog({ query: 'резюме' }).length, 1); });
...
test('Порог 10 символов применяется до проверки конкретности', () => { equal(assessText('123456789', 20).points, 0); equal(assessText('1234567890', 20).points, 20); equal(assessText('абвгдежзий', 20).points, 10); });
...
test('Баллы К1–К5 вычисляются из текста и отличаются от прежних ориентиров', () => equal(PARTICIPANT_CARDS.map(c => previewRating(c).score), [35,65,85,93,70]));
...
test('Повторное завершение не меняет дату, не создаёт событие и не начисляет баллы', () => { const s = newStore(); seedDemo(s); const p = s.listProposals()[0]; s.decideProposal(p.id, 'accepted'); const completed = s.completeProposal(p.id), count = s.exportData().events.length; equal(s.completeProposal(p.id).completedAt, completed.completedAt); equal(s.exportData().events.length, count); ok(!Object.hasOwn(completed, 'points')); throws(() => s.decideProposal(p.id, 'rejected'), 'ALREADY_COMPLETED'); });
